from django.shortcuts import render
from petstagram.pets.models import *
from petstagram.photos.models import *


def register_user(request):
    template_name = 'accounts/register-page.html'
    return render(request, template_name)


def login_user(request):
    template_name = 'accounts/login-page.html'
    return render(request, template_name)


def profile_details(request, pk: int):
    template_name = 'accounts/profile-details-page.html'

    all_pet_objects = Pet.objects.all()
    all_photo_objects = Photo.objects.all()

    context = {'all_pet_objects': all_pet_objects,
               'total_number_of_pets': all_pet_objects.count(),
               'total_number_of_photos': all_photo_objects.count()}

    return render(request, template_name, context)


def profile_edit(request, pk: int):
    template_name = 'accounts/profile-edit-page.html'
    return render(request, template_name)


def profile_delete(request, pk: int):
    template_name = 'accounts/profile-delete-page.html'
    return render(request, template_name)
